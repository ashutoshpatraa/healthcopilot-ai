...
def update_profile(payload: ProfileUpdateRequest, db: Session = Depends(db_session)) -> dict[str, object]:
    profile = db.query(MedicalProfile).first()
    if profile is None:
        profile = MedicalProfile(user_id=1)
        db.add(profile)
...
def predict(payload: PredictionRequest, db: Session = Depends(db_session)) -> PredictionResponse:
    ...
    db.add(Prediction(user_id=1, symptoms_csv=",".join(payload.symptoms), confidence=confidence, risk_score=risk_score, specialist=specialist))
    db.commit()
...
async def upload_report(file: UploadFile = File(...), db: Session = Depends(db_session)) -> ReportResponse:
    report = Report(user_id=1, file_name=file.filename or "report", extracted_text=f"Uploaded report: {file.filename}", summary="Report upload received and stored.")
    db.add(report)
    db.commit()
    db.refresh(report)
...
def chat(payload: ChatRequest, db: Session = Depends(db_session)) -> ChatResponse:
    db.add(ChatMessage(user_id=1, role="user", content=payload.message))
    reply = generate_medical_reply(payload.message)
    db.add(ChatMessage(user_id=1, role="assistant", content=reply))
    db.commit()
